...
function createProdutosAction($conn, $name, $email, $phone) {
	$createProdutosDb = createProdutosDb($conn, $name, $email, $phone);
	$message = $createProdutosDb == 1 ? 'success-create' : 'error-create';
	return header("Location: ./read.php?message=$message");
}

function updateProdutosAction($conn, $id, $name, $email, $phone) {
	$updateProdutosDb = updateProdutosDb($conn, $id, $name, $email, $phone);
